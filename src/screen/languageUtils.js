...
const saveLanguagePreference = async (language) => {
    ...
    if (user) {
        await firestore().collection('users').doc(user.uid).update({ language });
        await AsyncStorage.setItem('language', language); // Ayrıca AsyncStorage'a kaydedelim
    }
};

// Dil tercihlerini alma
const getLanguagePreference = async () => {
    const user = auth().currentUser;
    if (user) {
        const doc = await firestore().collection('users').doc(user.uid).get();
        return doc.data().language || 'en-US'; // Varsayılan dil
    }
    return 'en-US'; // Varsayılan dil
};
...
const Login = ({ navigation }) => {
    ...
    useEffect(() => {
        GoogleSignin.configure({
            ...
            androidClientId: ['YOUR_ANDROID_CLIENT_IDS'],
    ...
    useEffect(() => {
        const fetchLanguagePreference = async () => {
            ...
            if (storedLanguage) {
                ...
                // Burada dil ayarlama işlemini yapabilirsiniz
            } else {
    ...
    const handleGoogleLogin = async () => {
        try {
            ...
            // Save language preference if not set
            const language = await getLanguagePreference();
